function [postROIslice_filt,postBRAINslice_filt,postnuisanceslice_filt] = ROI_slice_mtr_post(iroi,rois,islice,ROIcolumn,postROI,postBRAIN,postnuisance,postart,postexc_o,TR,order)
            fprintf('-Working on %s slice %d/%d\n',rois{iroi},islice,length(ROIcolumn));
            ROIslice = postROI(:,islice);
            BRAINslice = postBRAIN;% bc each slice has its own sets of outliers etc. can't containimate the BRAIN
            nuisanceslice = postnuisance;
            ...
            ROIslice(postart,:) = NaN; %censor from timeseries
            BRAINslice(postart,:) = NaN;
            nuisanceslice(postart,:) = NaN;
            end
            ...
            postROIslice_filt = ROIslice_filt;
            postBRAINslice_filt = BRAINslice_filt;
            
            [tp v] = size(postROIslice_filt);
            [ttp vv] = size(postBRAINslice_filt);
            
            if tp == ttp
                   postruncode = [zeros(tp,1) ones(tp,1)];
            else
                error('postROIslice_filt & postBRAINslice_filt have different time points');
            end
            
            postnuisanceslice_filt = [nuisanceslice_filt postruncode];
